...
const ListaVacantes = ({ entidadID, vacantes, setVacantes, setVacante, setIsModalOpen, setSelectedPostulaciones, fetchPostulaciones }) => {
  ...
  const fetchVacantes = async () => {
    try {
      ...
      const response = await axios.get(`https://gestor-practicas-back.onrender.com/vacantePractica/${entidadID}`);
  ...
  const handleDelete = async (vacantePracticaID, event) => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`https://gestor-practicas-back.onrender.com/vacantePractica/${vacantePracticaID}`);
        Swal.fire(
          'Eliminado',

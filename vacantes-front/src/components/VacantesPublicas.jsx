...
const VacantesPublicas = () => {
  ...
  const aplicar = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post(`https://gestor-practicas-back.onrender.com/persons`, persona);
      let idPersona;
      try {
      ...
      const respuesta = await axios.post(`https://gestor-practicas-back.onrender.com/apply`, {
        job_id,
  ...
  const getVacantes = async () => {
    try {
      const { data } = await axios.get(`https://gestor-practicas-back.onrender.com/job/all/1/5`);
      setVacantes(data);
    } catch (err) {
      Swal.fire({
        position: "center",
                ...
                <div className="card-body d-flex flex-row">
                  <div className="flex-grow-1">
                    ...
                    <p className="card-text">
                      <img src={`data:image/png;base64,${item.logo}`} width={24} alt="logo" />{" "}
                      {item.company} Ubicación: {item.city} ({item.job_type})
                      <br />
                  ...
                  <button
                    ...
                    className="btn btn-success my-auto btn-lg"
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h1 className="modal-title fs-5" id="exampleModalLabel">
                Aplicar a la vacante <span className="text-success">{titulo}</span>
              </h1>
              <button
                type="button"
                className="btn-close"
            ...
            <div className="modal-body">
              <form>
                {foto && (
                  <div className="mb-3">
                    <img src={foto} height={100} alt="foto" />
                    <button
                      title="Quitar foto"
                      type="button"
                      onClick={limpiarFoto}
